...
import {GROUP_HORIZONTAL__PADDING, GROUP_VERTICAL__PADDING, NODE_HEIGHT, NODE_WIDTH,} from '@/constants/graph'
...
export function formatNodeInfoToNodeMeta(
  ...
  sortBy(inPorts, 'sequence').forEach((inPort) => {
    portItems.push({
      ...inPort,
      group: 'in',
  ...
  sortBy(outPorts, 'sequence').forEach((outPort) => {
    portItems.push({
      ...outPort,
      group: 'out',
...
export function formatGroupInfoToNodeMeta(
  ...
  if (isCollapsed && edges) {
    ...
    const includedNodeIds = includedNodes.map((nodeData) =>
...
export function formatGraphData(
  ...
  // 格式化算法组件节点
  const formattedNodes = nodes.map((nodeData) =>
    formatNodeInfoToNodeMeta(nodeData, inputPortConnectedMap),
  )

  // 格式化群组节点
  const formattedGroups = groups.map((groupData) =>
  ...
  return {
    nodes: [
      ...
      ...formattedGroups.filter(
        (group) => !!group.data.includedNodes.length,
      ),
    ],
